use crate::piston_window::{Context, G2d, Key};
...
const FOOD_COLOR: Color = [0.70, 0.00, 0.00, 1.00];
const BORDER_COLOR: Color = [0.00, 0.00, 0.00, 1.00];
const GAMEOVER_COLOR: Color = [0.80, 0.00, 0.00, 0.30];
...
impl Game {
    ...
    pub fn draw(&self, con: &Context, g: &mut G2d) {
        ...
        if self.game_over {
            draw_rectangle(GAMEOVER_COLOR, 0, 0, self.height, self.width, con, g);
        }
    }
    ...
    fn check_eating(&mut self) {
        let (head_x, head_y): (u16, u16) = self.snake.head_position();
